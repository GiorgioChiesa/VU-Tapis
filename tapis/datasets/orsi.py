...
class Orsi(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        paths = [os.path.join(self.video_root, path.lstrip("orsi_tensors/")) for path in image_paths]
        frames = [torch.load(p, map_location="cpu", weights_only=True) for p in paths]  # each [C, H, W]
        imgs = torch.stack(frames, dim=0)  # [T, C, H, W]
