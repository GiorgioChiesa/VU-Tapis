...
class SurgicalDataset(torch.utils.data.Dataset):
    ...
    def _load_data(self, cfg):
        ...
        boxes_and_labels = data_helper.load_boxes_and_labels(cfg, mode=self._split)

        assert len(boxes_and_labels) == len(self._image_paths)
        boxes_and_labels = [
            boxes_and_labels[self._video_idx_to_name[i]] for i in range(len(self._image_paths))
        ]
    ...
    def print_summary(self):
        ...
        logger.info(f"Split: {self._split}")
        logger.info(f"Number of videos: {len(self._image_paths)}")
        total_frames = sum(len(video_img_paths) for video_img_paths in self._image_paths)
        logger.info(f"Number of frames: {total_frames}")
        logger.info(f"Number of key frames: {len(self)}")
        if self.cfg.REGIONS.ENABLE:
            logger.info(f"Number of instances: {self._num_boxes_used}.")
