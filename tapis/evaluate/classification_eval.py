...
def eval_classification(task, coco_anns, preds, **kwargs):
    if kwargs.get("class_names", None) is not None:
        ...
        raise ValueError(
            f"Class names not found for task {task} in coco annotations. Please provide class_names in kwargs."
        )
    ...
    if isinstance(coco_anns, dict) and "annotations" in coco_anns and "images" in coco_anns:
        ...
        for img_name, pred_data in tqdm(preds.items()):
            ...
            for idx, ann in ann_by_image[img_name]:
                ...
                bin_labels.append(
                    label_binarize([ann_class], classes=list(range(0, num_classes)))[0]
                )
    ...
    plt.imsave(
        os.path.join(saving_dir, f"{mode}_preds_{task}>{threshold}.png"), bin_preds > threshold
    )
    plt.imsave(
...
def save_missmatches(
    preds,
    labels,
    task,
    class_name,
    output_dir="./temp",
    img_ann_dict: list = None,
    imgs_folder: str = None,
    **kwargs,
):
    ...
    for i, (pred_class, true_class, img_path) in enumerate(
        sorted(zip(preds, labels, img_ann_dict), key=lambda x: x[2])
    ):
            ...
            mismatches.append(
                {
                    ...
                    "image_name": img_path,
                    "video": video,
                    "predicted_class": class_name[pred_class]["name"]
                    if 0 <= pred_class < len(class_name)
                    else "Unknown",
                    "true_class": class_name[true_class]["name"]
                    if 0 <= true_class < len(class_name)
                    else "Unknown",
    ...
    mismatch_path = os.path.join(output_dir, task, f"{task}_missmatches.json")
    ...
    export_missmatches_per_classes(preds, labels, class_name, task, output_dir)

    if imgs_folder is not None:
        max_save_video = kwargs.get("max_save_video", 0)
        save_missmatches_videos(
            mismatches, os.path.join(output_dir, task), imgs_folder, max_save_video, **kwargs
        )
...
def export_missmatches_per_classes(preds, labels, class_name, task, output_dir="./temp"):
    ...
    precision_per_class = precision_score(
        labels, preds, labels=[c["id"] for c in class_name], average=None
    )
    ...
    csv_path = os.path.join(output_dir, task, f"{task}_per_class_metrics.csv")
...
def save_missmatches_videos(mismatches, output_dir, imgs_folder, max_video: int = 0, **kwargs):
    ...
    for error_type, error_mismatches in tqdm(
        errors_by_type.items(), total=len(errors_by_type), desc="Processing Mismatches"
    ):
        ...
        # Process each video
        for video, video_mismatches in tqdm(
            by_video.items(), total=len(by_video), desc=f"{video}:", leave=False
        ):
            # Sort by index
            ...
            for group in video_groups:
                ...
                frame_indices = [
                    idx if idx >= 0 else 0 for idx in frame_indices
                ]  # Handle negative indices:

                # Load frames and create video
                frames = loadpadding_frames(
                    imgs_folder,
                    group,
                    video,
...
def loadpadding_frames(imgs_folder, frames, video, time_window=16, mode="center", csv_folder=None):
    def get_time_range(indices):
        if mode == "center":
            start_idx = indices[0] - time_window // 2
            end_idx = indices[-1] + time_window // 2
        elif mode == "right":
            start_idx = indices[0]
            end_idx = indices[-1] + time_window
        elif mode == "left":
            start_idx = indices[0] - time_window
            end_idx = indices[-1]
        else:
            raise ValueError(f"Unsupported mode: {mode}")
        return max(start_idx, 0), max(end_idx, 0)

    def load_frames(base_dir, frame_paths):
        return [load_frame(os.path.join(base_dir, f)) for f in frame_paths]

    def build_frame_list_from_dir(base_dir):
        path = os.path.join(base_dir, frames[0]["image_name"])
        directory = os.path.dirname(path)
        list_frames = sorted(os.listdir(directory))
        frame_idxs = [list_frames.index(os.path.basename(frame["image_name"])) for frame in frames]
        start_idx, end_idx = get_time_range(frame_idxs)
        return directory, list_frames[start_idx:end_idx]

    if csv_folder:
        try:
            regexcsv_path = os.path.join(csv_folder, "**", video, "**", "*all*.csv")
            csv_paths = glob(regexcsv_path, recursive=True)
            assert len(csv_paths) == 1, (
                f"Expected exactly one CSV file for video {video}, regex: {regexcsv_path}, found: {csv_paths}"
            )
            list_frames = pd.read_csv(csv_paths[0])["frame_path"].tolist()
            frame_idxs = [list_frames.index(frame["image_name"]) for frame in frames]
            start_idx, end_idx = get_time_range(frame_idxs)
            return load_frames(imgs_folder, list_frames[start_idx:end_idx])
        except Exception:
            print(
                f"Could not load frames from CSV for video {video}, falling back to directory listing."
            )
            pass

    directory, frames_list = build_frame_list_from_dir(imgs_folder)
    return [load_frame(os.path.join(directory, f)) for f in frames_list]


def load_frame(frame_path):
    frame_path = frame_path.replace("/orsi_tensors/orsi_tensors/", "/orsi_tensors/")
    try:
        ...
        if frame is not None:
            ...
            frame_path = (
                frame_path.replace("/nas_private/", "/")
                .replace("/orsi/", "/orsi_tensors/")
                .replace(".jpg", ".pt")
            )
...
def plot_video_missmatches(
    mismatches_by_video: dict,
    task: str,
    output_dir: str = "./temp",
    maxCounter: dict = {},
    **kwargs,
    ...
    # Save plot
    plot_path = os.path.join(output_dir, task, f"{task}_missmatches_frames_plot.png")
...
def save_confusion_matrix(
    preds, labels, class_name: list = [], path: str = "./temp/confusion_matrix.png", **kwargs
):
    ...
    if (~valid_mask).sum() > 0:
        logging.warning(
            f"Found {(~valid_mask).sum()} invalid labels (-1) in confusion matrix. Filtering them out."
        )
    ...
    all_classes = np.concatenate(
        [
            np.argmax(preds, axis=1) if len(preds.shape) == 2 and preds.shape[1] > 1 else preds,
...
def eval_presence(task, coco_anns, preds, img_ann_dict, **kwargs):
    ...
    for idx, img in tqdm(enumerate(coco_anns["images"]), total=len(coco_anns["images"])):
        ...
        assert all(type(label_list[0]) == type(lab_item) for lab_item in label_list), (
            f"Inconsistent label type {label_list} in frame {img['file_name']}"
        )
        ...
        if isinstance(label_list[0], list):
            ...
            raise ValueError(
                f"Do not support annotation {label_list[0]} of type {type(label_list[0])} in frame {img['file_name']}"
            )
    ...
    ap = {}
    for c in range(0, num_classes):
        precision[c], recall[c], threshs[c] = precision_recall_curve(
            bin_labels[:, c], bin_preds[:, c]
        )
        ap[c] = average_precision_score(bin_labels[:, c], bin_preds[:, c])
